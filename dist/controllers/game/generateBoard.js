...
var _checkMoves = require("./checkMoves");
...
const hardMode = (board, rng) => {
  ...
  for (let i = 0; i < moveCount; i++) {
    ...
    if (board.tiles[move[1]][move[0]] === 1 || board.tiles[move[1]][move[0]] === 0) {
      board.tiles[move[1]][move[0]] = (0, _checkMoves.flip)(board.tiles[move[1]][move[0]]);
      // tile UP
      if (move[1] + 1 <= board.tiles.length - 1) {
        board.tiles[move[1] + 1][move[0]] = (0, _checkMoves.flip)(board.tiles[move[1] + 1][move[0]]);
      }

      // tile RIGHT
      if (move[0] + 1 <= board.tiles.length - 1) {
        board.tiles[move[1]][move[0] + 1] = (0, _checkMoves.flip)(board.tiles[move[1]][move[0] + 1]);
      }
...
const generateBoard = (size, seed, easyMode = false) => {
  if (!seed) {
    seed = (0, _md.default)(`${Date.now() + size + Math.random()}`).slice(0, -16);
  }
  let board = {
    size: size,
    tiles: [],
